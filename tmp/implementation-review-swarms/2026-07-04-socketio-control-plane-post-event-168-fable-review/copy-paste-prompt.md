...
OpenClaw, Worker, VM, default e2e, and `pnpm check` refresh, plus Event 213
controller-host-action validation fixes.
...
8. terminal e2e freshness after Event 200, OpenClaw freshness after Event 205,
    ...
    proof, Event 212 terminal OpenClaw/Worker/VM/default e2e proof, Event 213
    controller-host-action validation proof, and remaining stale beta
...
37. Event 213 controller-host-action validation fixes:
    - Local review accepted two important validation findings.
    - Static Tool Portal materialization validation now enables
      `controller_host_action` when OpenClaw `zoneGit` is configured.
    - `--mcp-live` no longer treats `controller_host_action` as an upstream MCP
      provider namespace when `zoneGit` is enabled.
    - Focused config-validation integration passed 1 file / 30 tests.
    - Full integration passed 28 files / 443 tests.
    - Fresh `pnpm check` passed 10 checks / 0 failed.
