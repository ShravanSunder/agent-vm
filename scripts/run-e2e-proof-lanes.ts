...
export async function runE2eProofLanes(
	...
	const laneResultsById = new Map<E2eProofLaneId, E2eProofLaneResult>();
	const runnableLanes: E2eProofLane[] = [];
	for (const lane of lanes) {
		const skipReason = resolveE2eProofLaneSkipReason(lane, env);
		if (skipReason !== undefined) {
			laneResultsById.set(lane.id, createSkippedE2eProofLaneResult(lane, skipReason));
			continue;
		}
		runnableLanes.push(lane);
	}
	for (const laneBatch of createE2eProofLaneBatches({
		isolateHostDockerLane,
		lanes: runnableLanes,
	})) {
		...
		for (const result of batchResults) {
			laneResultsById.set(result.lane.id, result);
		}
	}
	const results = lanes.map((lane) => {
		const result = laneResultsById.get(lane.id);
		if (result === undefined) {
			throw new Error(`E2E proof lane '${lane.id}' did not produce a result.`);
		}
		return result;
	});
