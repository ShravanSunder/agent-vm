...
describe('portal architecture audit', () => {
	...
	it('rejects controller-owned OpenClaw local process supervision while allowing Worker direct processes', () => {
		// Arrange / Act
		const violations = collectPortalArchitectureViolations({
			files: [
				{
					...
					sourceText: 'export interface OpenClawProcessSupervisorRequest {}\n',
				},
				{
					filePath:
						'packages/agent-vm/src/controller/process-supervisor/openclaw-process-supervisor.integration.test.ts',
					sourceText: 'describe("managed VM OpenClaw process supervisor", () => {});\n',
				},
				{
					filePath:
						'packages/agent-vm/src/controller/process-supervisor/openclaw-process-supervisor.ts',
					sourceText: 'export interface OpenClawProcessSupervisor {}\n',
				},
				{
					filePath:
						'packages/agent-vm/src/controller/process-supervisor/openclaw-process-supervisor.unit.test.ts',
					sourceText: 'describe("OpenClaw process supervisor", () => {});\n',
				},
				{
					filePath:
						'packages/agent-vm/src/controller/reliability/testing/openclaw-process-reliability-fault-handler.ts',
					sourceText: 'export function createOpenClawProcessReliabilityFaultHandler(): void {}\n',
				},
				{
					filePath:
						'packages/agent-vm/src/controller/reliability/testing/openclaw-process-reliability-fault-handler.unit.test.ts',
					sourceText: 'describe("OpenClaw process reliability fault handler", () => {});\n',
				},
				{
					filePath:
						'packages/agent-vm/src/controller/reliability/testing/openclaw-process-reliability-fault-target-registry.ts',
					sourceText:
						'export function createOpenClawProcessReliabilityFaultTargetRegistry(): void {}\n',
				},
				{
					filePath:
						'packages/agent-vm/src/controller/reliability/testing/openclaw-process-reliability-fault-target-registry.unit.test.ts',
					sourceText: 'describe("OpenClaw process reliability target registry", () => {});\n',
				},
				{
					filePath: 'packages/agent-vm/src/controller/zone-runtimes/openclaw-process-recovery.ts',
					sourceText: 'export function createOpenClawProcessRecoveryCoordinator(): void {}\n',
				},
				{
					filePath:
						'packages/agent-vm/src/controller/zone-runtimes/openclaw-process-recovery.unit.test.ts',
					sourceText: 'describe("OpenClaw process recovery", () => {});\n',
				},
				{
					filePath: 'packages/agent-vm/src/gateway/openclaw-gateway-process-epoch-owner.ts',
					sourceText: 'export interface OpenClawGatewayProcessEpochOwner {}\n',
				},
				{
					filePath:
						'packages/agent-vm/src/gateway/openclaw-gateway-process-epoch-owner.unit.test.ts',
					sourceText: 'describe("OpenClaw Gateway process epoch owner", () => {});\n',
				},
				{
					filePath: 'packages/agent-vm/src/gateway/gateway-zone-orchestrator.ts',
					sourceText:
						"import { createManagedVmOpenClawProcessSupervisorPorts } from '../controller/process-supervisor/openclaw-process-supervisor.js';\nimport { createOpenClawGatewayProcessEpochOwner } from './openclaw-gateway-process-epoch-owner.js';\nconst recovery = createOpenClawProcessRecoveryCoordinator();\nconst target = createOpenClawProcessReliabilityFaultTargetRegistry();\n",
				},
				{
					filePath: 'packages/agent-vm/src/gateway/gateway-zone-support.ts',
					sourceText:
						"import type { OpenClawProcessEpochLossBarrier } from './openclaw-gateway-process-epoch-owner.js';\n",
				},
				{
					filePath: 'packages/agent-vm/src/index.ts',
					sourceText: 'export { resolveOpenClawProcessSupervisorStateMount };\n',
				},
				{
					filePath: 'packages/gateway-lifecycle/src/gateway-process-spec.ts',
					sourceText:
						'export interface GatewayProcessSpec { readonly bootstrapCommand: string; readonly startCommand: string; }\n',
				},
				{
					filePath: 'packages/worker-gateway/src/worker-lifecycle.ts',
					sourceText:
						"export const worker = { executionModel: 'direct-process', processSpec: { bootstrapCommand: 'prepare-worker', startCommand: 'start-worker' } };\n",
				},
				{
					filePath: 'packages/agent-vm/src/gateway/worker-direct-process-runner.ts',
					sourceText:
						'await managedVm.exec([processSpec.bootstrapCommand]);\nawait managedVm.exec([processSpec.startCommand]);\n',
				},
				{
					filePath: 'docs/specs/2026-07-12-rejected-openclaw-process-supervisor.md',
					sourceText:
						'Historical design record: OpenClawProcessSupervisor and a same-VM successor were rejected.\n',
				},
			],
		});
		...
		// Assert
		expect(violations).toEqual([
			'packages/agent-vm/src/controller/process-supervisor/openclaw-process-supervisor-contracts.ts: managed OpenClaw must not retain controller-owned process supervisor source or proof',
			'packages/agent-vm/src/controller/process-supervisor/openclaw-process-supervisor.integration.test.ts: managed OpenClaw must not retain controller-owned process supervisor source or proof',
			'packages/agent-vm/src/controller/process-supervisor/openclaw-process-supervisor.ts: managed OpenClaw must not retain controller-owned process supervisor source or proof',
			'packages/agent-vm/src/controller/process-supervisor/openclaw-process-supervisor.unit.test.ts: managed OpenClaw must not retain controller-owned process supervisor source or proof',
			'packages/agent-vm/src/controller/reliability/testing/openclaw-process-reliability-fault-handler.ts: managed OpenClaw reliability faults must target the Gateway VM, not an OpenClaw process',
			'packages/agent-vm/src/controller/reliability/testing/openclaw-process-reliability-fault-handler.unit.test.ts: managed OpenClaw reliability faults must target the Gateway VM, not an OpenClaw process',
			'packages/agent-vm/src/controller/reliability/testing/openclaw-process-reliability-fault-target-registry.ts: managed OpenClaw reliability faults must target the Gateway VM, not an OpenClaw process',
			'packages/agent-vm/src/controller/reliability/testing/openclaw-process-reliability-fault-target-registry.unit.test.ts: managed OpenClaw reliability faults must target the Gateway VM, not an OpenClaw process',
			'packages/agent-vm/src/controller/zone-runtimes/openclaw-process-recovery.ts: managed OpenClaw must not retain process-scoped recovery or local-successor source or proof',
			'packages/agent-vm/src/controller/zone-runtimes/openclaw-process-recovery.unit.test.ts: managed OpenClaw must not retain process-scoped recovery or local-successor source or proof',
			'packages/agent-vm/src/gateway/gateway-zone-orchestrator.ts: managed OpenClaw must not retain controller-owned process supervisor consumers',
			'packages/agent-vm/src/gateway/gateway-zone-orchestrator.ts: managed OpenClaw must not retain process-scoped recovery or local-successor consumers',
			'packages/agent-vm/src/gateway/gateway-zone-orchestrator.ts: managed OpenClaw must not retain same-VM process-epoch owner consumers',
			'packages/agent-vm/src/gateway/gateway-zone-orchestrator.ts: managed OpenClaw reliability faults must target the Gateway VM, not an OpenClaw process',
			'packages/agent-vm/src/gateway/gateway-zone-support.ts: managed OpenClaw must not retain same-VM process-epoch owner consumers',
			'packages/agent-vm/src/gateway/openclaw-gateway-process-epoch-owner.ts: managed OpenClaw must not retain same-VM process-epoch owner source or proof',
			'packages/agent-vm/src/gateway/openclaw-gateway-process-epoch-owner.unit.test.ts: managed OpenClaw must not retain same-VM process-epoch owner source or proof',
			'packages/agent-vm/src/index.ts: managed OpenClaw must not retain controller-owned process supervisor consumers',
		]);
	});
