...
export const instructionTextSchema = z.string();
const runtimeInstructionTextSchema = z.string().min(1);
...
export const workerConfigSchema = z
	.object({
		runtimeInstructions: runtimeInstructionTextSchema,
		commonAgentInstructions: nullableInstructionTextSchema.optional(),
		defaults: z
			.object({
...
export const workerConfigDraftSchema = workerConfigSchema.extend({
	runtimeInstructions: runtimeInstructionTextSchema.optional(),
});
...
export type WorkerConfigDraft = z.infer<typeof workerConfigDraftSchema>;
...
function buildDefaultWorkerConfigInput(): Record<string, unknown> {
	return {
		runtimeInstructions: 'Runtime instructions are generated by the controller.',
...
export async function resolveWorkerConfigInstructionReferences(
	...
	await resolveOptionalInstructionField(
		resolvedConfig,
...
async function loadWorkerConfigWithSchema<TConfig>(
	configPath: string | undefined,
	schema: z.ZodType<TConfig>,
): Promise<TConfig> {
	if (!configPath) {
		return schema.parse(buildDefaultWorkerConfigInput());
	}
	...
	try {
		...
		const resolvedRaw = isObjectRecord(raw)
			...
			: raw;
		return schema.parse(resolvedRaw);
	} catch (error) {
		const message = error instanceof Error ? error.message : String(error);
...
export async function loadWorkerConfig(configPath?: string): Promise<WorkerConfig> {
	return await loadWorkerConfigWithSchema(configPath, workerConfigSchema);
}

export async function loadWorkerConfigDraft(configPath?: string): Promise<WorkerConfigDraft> {
	return await loadWorkerConfigWithSchema(configPath, workerConfigDraftSchema);
}
