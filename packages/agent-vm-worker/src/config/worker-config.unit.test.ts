...
describe('worker-config', () => {
	...
	describe('loadWorkerConfig', () => {
		...
		it('loads JSONC worker config with comments and trailing commas', async () => {
			const configPath = join(tempDir, 'worker.jsonc');
			await writeFile(
				configPath,
				[
					'{',
					'  // Shared runtime instructions are generated by the controller.',
					'  "runtimeInstructions": "runtime facts",',
					'  "commonAgentInstructions": null,',
					'  "defaults": { "provider": "codex", "model": "latest-medium" },',
					`  "phases": ${JSON.stringify(baseValidConfig().phases)},`,
					'  "mcpServers": [],',
					'  "verification": [],',
