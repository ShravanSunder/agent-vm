...
describe('gateway control admission pressure E2E actuator', () => {
	...
	it('holds bounded non-authoritative work and drains the real executor', async () => {
		const fixture = createFixture();
		...
		expect(before.ingress.activeByClass.diagnostic).toBe(0);
		expect(pressure.admissions.filter((result) => result.status === 'dropped')).toHaveLength(2);
		expect(pressure.snapshot.ingress.activeByClass.diagnostic).toBe(1);
	...
	it('registers the real Gateway Runtime application-message admission owner', () => {
		vi.stubEnv(AGENT_VM_E2E_CONTROL_ADMISSION_PRESSURE_ENV, '1');
