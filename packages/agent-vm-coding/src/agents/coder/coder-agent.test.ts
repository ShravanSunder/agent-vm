...
import type { CodexClient, CodexRunResult, CodexThread, StructuredInput } from '../shared-types.js';
import { createCoderAgent } from './coder-agent.js';

class MockCodexThread implements CodexThread {
	constructor(
		private readonly threadId: string,
		private readonly runImpl: (input: StructuredInput[]) => Promise<CodexRunResult>,
	) {}
...
describe('coder-agent', () => {
	it('implement creates a thread and returns the result', async () => {
		...
		const result = await agent.implement([{ type: 'text', text: 'Implement this feature' }]);
	...
	it('fix reuses the same thread', async () => {
		...
		expect(result.threadId).toBe('thread-1');
		expect(client.getHistory().map((entry) => entry.threadId)).toEqual(['thread-1', 'thread-1']);
	});
	...
	it('resumeOrRebuild resumes the existing thread when possible', async () => {
		...
		await agent.implement([{ type: 'text', text: 'Implement' }]);
		await agent.resumeOrRebuild('thread-1', [{ type: 'text', text: 'Resume context' }]);
	...
	it('resumeOrRebuild rebuilds when resume fails', async () => {
		...
		await agent.resumeOrRebuild('thread-1', [{ type: 'text', text: 'Replay context' }]);
		const result = await agent.fix([{ type: 'text', text: 'Continue' }]);
	...
	it('resumeOrRebuild rethrows unexpected resume errors', async () => {
		...
		await expect(
			agent.resumeOrRebuild('thread-1', [{ type: 'text', text: 'Replay context' }]),
