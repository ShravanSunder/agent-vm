...
import {
  createTaskConfigFixture,
  createTaskStateFixture,
} from "./tests/support/task-fixtures.js";
...
describe("server", () => {
  ...
  it("POST /tasks returns 409 when another task is active", async () => {
    const app = createApp(
      createMockDeps({ getActiveTaskId: () => "active-task-123" }),
    );
  ...
  it("POST /tasks/:id/followup only accepts awaiting-followup", async () => {
    const app = createApp(
      createMockDeps({
        getTaskState: () =>
          createMockTaskState({ status: "implementing" }),
      }),
    );
  ...
  it("POST /tasks/:id/followup returns 200 when awaiting-followup", async () => {
    const app = createApp(
      createMockDeps({
        getTaskState: () =>
          createMockTaskState({ status: "awaiting-followup" }),
      }),
    );
