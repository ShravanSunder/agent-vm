...
describe('git-operations', () => {
	...
	describe('buildPushUrl', () => {
		it('uses the actual GITHUB_TOKEN instead of leaving a literal placeholder', () => {
			...
			expect(url).toBe('https://x-access-token:token-value@github.com/owner/repo.git');
		...
		it('handles organization/repository format', () => {
			...
			expect(url).toBe('https://x-access-token:token@github.com/myorg/myproject.git');
			process.env.GITHUB_TOKEN = previousToken;
		});
	...
	describe('configureGit', () => {
		it('configures git with repo-local settings (not global)', async () => {
			...
			try {
				...
				const emailResult = await execa('git', ['config', '--local', 'user.email'], {
					cwd: tempDir,
				});
				expect(emailResult.stdout.trim()).toBe('test@example.com');

				const nameResult = await execa('git', ['config', '--local', 'user.name'], { cwd: tempDir });
				expect(nameResult.stdout.trim()).toBe('Test User');

				// http.version should also be local
				const httpResult = await execa('git', ['config', '--local', 'http.version'], {
					cwd: tempDir,
				});
