...
export interface MockPlannerAgent extends PlannerAgent {
	readonly planCalls: StructuredInput[][];
	readonly reviseCalls: StructuredInput[][];
}

export interface MockCoderAgent extends CoderAgent {
...
export function createAgentRunResult(response: string, threadId: string): AgentRunResult {
	return {
		response,
		tokenCount: 100,
		threadId,
...
export function createPlanReviewer(
	responses: readonly ReviewResult[] = [{ approved: true, comments: [], summary: 'Looks good' }],
...
export function createCoderAgent(options?: {
	...
	return {
		async implement(input: readonly StructuredInput[]): Promise<AgentRunResult> {
			...
			return createAgentRunResult(options?.implementResponse ?? 'Implemented', threadId);
		},
		async fix(input: readonly StructuredInput[]): Promise<AgentRunResult> {
			fixCalls.push([...input]);
			await options?.callbacks?.onFix?.();
			return createAgentRunResult(options?.fixResponse ?? 'Fixed', threadId ?? 'coder-thread-1');
		},
