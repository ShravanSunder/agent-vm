...
vi.mock('../../git/git-operations.js', async (importOriginal) => {
	...
	return {
		...
		createPullRequest: vi.fn(async (): Promise<string> => 'https://github.com/test/repo/pull/42'),
...
function createPlannerAgent(
	...
	return {
		...
		async revise(_input: readonly StructuredInput[]): Promise<AgentRunResult> {
			return createAgentRunResult(revisedResponse, threadId ?? 'planner-thread-1');
		},
...
function createCoderAgent(callbacks?: {
