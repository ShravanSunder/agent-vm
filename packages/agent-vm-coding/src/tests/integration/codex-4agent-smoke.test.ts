...
vi.mock("../../git/git-operations.js", async (importOriginal) => {
  ...
  return {
    ...
    createPullRequest: vi.fn(
      async (): Promise<string> => "https://github.com/test/repo/pull/99",
    ),
  };
});
...
describe.skipIf(!hasApiKey)(
  "4-agent Codex smoke test (requires CODEX_API_KEY)",
  () => {
    ...
    it(
      "should complete plan + implement cycle with real Codex and reach awaiting-followup",
      async () => {
        ...
        const planReviewerAgent = createPlanReviewerAgent(
          { model: "gpt-5.4-mini" },
          codexClient,
        );
        ...
        while (Date.now() - start < maxWaitMs) {
          ...
          if (
            state?.status === "awaiting-followup" ||
            state?.status === "failed"
          ) {
            break;
          }
