...
function createAgentRunResult(response: string, threadId: string): AgentRunResult {
	return {
		response,
		tokenCount: 100,
		threadId,
...
function createPlannerAgent(overrides?: Partial<PlannerAgent>): PlannerAgent & {
...
function createCoderAgent(overrides?: Partial<CoderAgent>): CoderAgent & {
...
describe('coordinator', () => {
	...
	it('retries after sanity failure and preserves the actual exit codes in retry context', async () => {
		...
		const retryPrompt = coderAgent.fixCalls[0]?.find((item) => item.type === 'text');
	...
	it('sanitizes token-bearing failures before persisting task-failed', async () => {
		...
		const logContents = readFileSync(join(stateDir, 'tasks', `${taskId}.jsonl`), 'utf-8');
