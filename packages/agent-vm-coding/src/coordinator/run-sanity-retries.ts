...
export async function runSanityRetries(
	...
	eventRecorder: {
		readonly emit: (
			taskId: string,
			event: import('../state/task-event-types.js').TaskEvent,
		) => void;
	...
	for (let attempt = 1; attempt <= state.config.maxSanityRetries; attempt += 1) {
		...
		const passed = verifyResult.testStatus === 'passed' && verifyResult.lintStatus === 'passed';
		...
		if (attempt === state.config.maxSanityRetries) {
			eventRecorder.recordTaskFailure(taskId, `Sanity check failed after ${attempt} attempts`);
			return false;
		}
