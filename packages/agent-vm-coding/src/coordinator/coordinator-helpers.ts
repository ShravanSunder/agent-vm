...
import type { RetryPromptContext, TaskConfig, TaskEvent } from '../state/task-event-types.js';
...
export function buildTaskConfig(input: CreateTaskInput, config: CodingGatewayConfig): TaskConfig {
	return {
		prompt: input.prompt,
...
export function createRetryContext(state: TaskState, filesChanged: string): RetryPromptContext {
	return {
