...
import { resolveSkillInputs, type SkillName } from '../agents/skill-registry.js';
import type { RepoContext } from '../context/gather-context.js';
import type { RetryPromptContext } from '../state/task-event-types.js';

function withSkills(text: string, skills: readonly SkillName[]): readonly StructuredInput[] {
...
export function buildPlannerPrompt(input: PlannerPromptInput): readonly StructuredInput[] {
	return withSkills(
		[
...
export function buildPlanRevisionPrompt(
	...
	return withSkills(`Revise the plan using this feedback:\n\n${input.reviewSummary}`, input.skills);
}

export function buildCoderImplementPrompt(
	input: CoderImplementPromptInput,
): readonly StructuredInput[] {
	return withSkills(`Implement the approved plan:\n\n${input.plan}`, input.skills);
}

export function buildCoderFixPrompt(input: CoderFixPromptInput): readonly StructuredInput[] {
	return withSkills(`Address the following feedback:\n\n${input.reviewSummary}`, input.skills);
}
...
export function buildCodeReviewPrompt(input: CodeReviewPromptInput): readonly StructuredInput[] {
	return withSkills(
		[
