...
async function runTask(
  ...
  try {
    ...
    if (initialState.followupPrompt === null) {
      // eslint-disable-next-line no-console
      console.log(`[coordinator] task ${taskId}: planning (loop 1/${initialState.config.maxPlanReviewLoops})`);
    ...
    // eslint-disable-next-line no-console
    console.log(`[coordinator] task ${taskId}: sanity check 1/${initialState.config.maxSanityRetries}`);
    ...
    for (
      let loop = 1;
      loop <= initialState.config.maxCodeReviewLoops;
      loop += 1
    ) {
      ...
      // eslint-disable-next-line no-console
      console.log(`[coordinator] task ${taskId}: code review (loop ${loop}/${initialState.config.maxCodeReviewLoops})`);
      ...
      // eslint-disable-next-line no-console
      console.log(`[coordinator] task ${taskId}: sanity check ${loop + 1}/${initialState.config.maxSanityRetries}`);
      const retryPassed = await runSanityRetries(
        taskId,
        deps,
        workspaceDir,
        tasks,
  ...
  return {
    ...
    async submitFollowup(
      ...
      if (state.status !== "awaiting-followup") {
        throw new Error(
          `Task ${taskId} is not awaiting followup: ${state.status}`,
        );
      }
