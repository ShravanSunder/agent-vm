...
export const codingGatewayConfigSchema = z.object({
	...
	plannerSkills: z.array(skillNameSchema).default(['writing-plans', 'brainstorming']),
	planReviewerSkills: z.array(skillNameSchema).default(['generic-plan-review']),
	coderSkills: z
		.array(skillNameSchema)
		.default(['test-driven-development', 'verification-before-completion']),
	codeReviewerSkills: z.array(skillNameSchema).default(['generic-code-review']),
	...
	commitCoAuthor: z.string().min(1).default('agent-vm-coding <noreply@agent-vm>'),
