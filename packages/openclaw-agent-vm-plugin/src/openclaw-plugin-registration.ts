...
const plugin = {
	...
	register(api: {
		...
		const sdkPath =
			'/usr/local/lib/node_modules/openclaw/dist/plugin-sdk/sandbox.js';
		const sdkPromise = import(sdkPath).then(
			(sdkRaw: Record<string, unknown>) => {
				...
				const sshHelpers: SshHelpers = {
					...
					createRemoteShellSandboxFsBridge:
						sdkRaw.createRemoteShellSandboxFsBridge,
					createSshSandboxSessionFromSettings:
						sdkRaw.createSshSandboxSessionFromSettings,
					...(typeof sdkRaw.disposeSshSandboxSession === 'function'
						? {
								disposeSshSandboxSession:
									sdkRaw.disposeSshSandboxSession as (
		...
		sdkPromise.catch((error: unknown) => {
			...
			process.stderr.write(
				`[gondolin] failed to load OpenClaw SDK: ${message}\n`,
			);
		});
	},
};
