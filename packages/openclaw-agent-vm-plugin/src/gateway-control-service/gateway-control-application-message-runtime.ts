...
import { GatewayControlSessionUnavailableError } from './gateway-control-service-contracts.js';
import type {
		...
		socket.on('control:message', (envelopePayload, payload, acknowledge) => {
			void (async () => {
				try {
					...
					assertControlEnvelopeMatchesDomainMessage(
						envelope,
						applicationMessageHandler.messageIdentity({ envelope, payload: gatewayPayload }),
					);
					assertGatewayControlEnvelopeDeliveryPolicy(envelope);
					const classification = classifyGatewayControlAdmission({
					...
					const submission = admissionRuntime.ingress.submit({
						...
						execute: async () => {
							...
							if (envelope.kind !== 'command' || responsePayload === undefined) {
								return;
							}
