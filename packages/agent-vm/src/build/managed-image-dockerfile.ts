...
function renderManagedDockerfile(props: {
	...
	const lines = [
		`FROM ${baseImageRepositoryByBase[props.base]}:${props.packageVersion}`,
		'',
...
const packageVersionSchema = z.object({ version: z.string().min(1) }).passthrough();

export async function resolveAgentVmPackageVersion(): Promise<string> {
	...
	for (;;) {
		...
		try {
			const packageJson = packageVersionSchema.parse(await loadJsonConfigFile(packageJsonPath));
		...
		if (parentDirectory === searchDirectory) {
			throw new Error('Unable to resolve @agent-vm/agent-vm package version.');
		}
