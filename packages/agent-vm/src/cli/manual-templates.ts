...
Managed Gateway boot starts one common Tool Portal service process beside the Hermes framework process. The Hermes integration authenticates its native profile and uses one GatewayRuntimeClient over the private UDS. ToolPortalService owns policy, controller bindings, per-agent strict SSH, active uses, and Sandbox operations; Hermes never receives lease ids or SSH material.
	...
	All portal responses are { ok, items, diagnostics? }. Each item has id and status. Successful items use { status: "ok", value }; failed items use { status: "error", error }.
		...
		controller_execution operations are registered_action or configured_cli. Configured CLI binds controller_host or a fresh one-shot ephemeral_managed_vm, never the leased Tool VM. An ephemeral target's imageReference is a Managed VM image recipe path relative to tool-portal.config.jsonc; Gateway startup prepares it and fails closed before protected execution if no immutable reference and fingerprint are produced. Prepared image details stay controller-only. quick operations run for at most 5 seconds. open operations default to 120 seconds and accept a caller timeout up to 8 hours. tool_vm_runner remains direct Gateway-to-leased-Tool-VM SSH and does not send a per-command controller execution RPC.
