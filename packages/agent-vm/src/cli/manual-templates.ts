...
Agent-vm scaffolds OpenClaw defaults that make the deployment usable without hand-editing Dockerfiles or OpenClaw internals.
	...
	Managed OpenClaw gateway images install @agent-vm/openclaw-agent-vm-plugin and register it as the gondolin extension.
		Managed OpenClaw gateway images install @agent-vm/openclaw-mcp-portal-plugin and @agent-vm/mcp-portal. The plugin supervises the standalone portal subprocess inside the gateway VM and OpenClaw connects to it through per-agent mcp.servers entries.
...
MCP Portal is an MCP server facade over deployment-owned upstream MCP servers.
	...
	Each agent receives a separate Streamable HTTP portal endpoint at /agents/<agentId>/mcp and a named profile in mcp-portal.config.jsonc. mcp.config.jsonc owns the upstream MCP providers; mcp-portal.config.jsonc owns the portal server access header, agent profile assignments, and profile policies. Namespace exposure defaults to deny-all unless config enables namespaces. Denied tools do not enter the agent's catalog or search index.

	Authored config is JSONC. $schema points at local config/schemas/*.schema.json files generated by agent-vm init or agent-vm migrate mcp-portal. schemaVersion is the runtime migration gate.

	MCP JSON Schema is canonical. Zod is derived for validation and helper code. Invalid call arguments and schemas that cannot be converted return structured errors without calling upstream. Tool VMs can use the agent-vm-mcp-portal helper package but do not receive upstream MCP credentials.

	Read-only/destructive annotations are trusted only for configured namespaces. Untrusted upstream tools require approval unless explicitly allowlisted by policy. Approved calls receive an unadvertised portalApprovalToken signed by the OpenClaw plugin and verified by the portal subprocess.
