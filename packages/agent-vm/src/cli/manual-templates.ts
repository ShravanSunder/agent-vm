...
Managed Gateway boot starts one common Tool Portal service process beside the Hermes framework process. The Hermes integration authenticates its native profile and uses one GatewayRuntimeClient over the private UDS. ToolPortalService owns policy, controller bindings, per-agent strict SSH, active uses, and Sandbox operations; Hermes never receives lease ids or SSH material.
	...
	All portal responses are { ok, items, diagnostics? }. Each item has id and status. Successful items use { status: "ok", value }; failed items use { status: "error", error }.
		...
		tool_vm_runner command.cli is a separate strict configuration variant for one executable already installed in the caller's current leased Tool VM. Configuration owns the absolute executable, safeHelp, optional bounded metadata, work-relative workingDirectory, timeout class, and output policy. The caller supplies unrestricted tokenized argv and optional stdin; Agent VM adds no admitted command grammar and dispatches [executable, ...argv] through the existing current-generation strict-SSH path. Only this variant may use advisoryHints.hintDeny or advisoryHints.hintRequiresApproval. Those names are literal: they affect only tool_portal_call and do not restrict terminal or Python in the same Tool VM. Precedence is hintDeny, then hintRequiresApproval, then unmatched direct execution. A visible hintRequiresApproval requires zone approvalAccess while the namespace command.cli baseline remains calls.withoutApproval.

		Configured CLI binds controller_host or a reusable credentialed ephemeral_managed_vm, never the leased Tool VM. Here ephemeral means controller-created, non-durable, and idle-retired; it does not mean one VM per RPC. Each Managed target declares runtimeId, credentialBinding, credentialFiles, and credentialEnvironment. Agent credentialBindings hold controller-only 1Password file refs; the controller maps them into read-only memory paths while ordinary CLI config/state/cache remains on disposable COW. One agent/runtime runs one command; concurrent calls return retryable busy and are never queued. Compatible calls reuse the VM until 15 idle minutes, but every call gets current policy and approval, and cancellation or zone shutdown before active admission produces no guest process. imageReference remains a recipe path prepared at Gateway startup. Prepared image details, credential refs, file paths, runtime ids, and VM authority stay out of Gateway-safe projection. quick operations run for at most 5 seconds. open operations default to 120 seconds and accept a caller timeout up to 8 hours. tool_vm_runner remains direct Gateway-to-leased-Tool-VM strict SSH and does not send a per-command controller execution RPC.

		For immediate replacement of a value behind an unchanged 1Password ref, run agent-vm controller credential-runtime retire --zone <zone> --agent <agentId> --runtime <runtimeId>. Without --force, an active runtime remains active. --force cancels its current command before exact cleanup. The command uses existing zone adminAccess and exposes no credential, VM, or lease identity.
