...
config/gateways/<zone>/oauth.config.jsonc optionally enables controller-owned Google OAuth for that managed Hermes zone. It defines Clerk human identity, human owners, policy editors, and Google application-family bindings. Agent role policy belongs in the sibling Tool Portal profiles, not in an OAuth agents map. Accounts and separate agent/account/application connections are enrolled dynamically. Google credential payloads are envelope-encrypted in controller-state SQLite using a 1Password-held wrapping key; Clerk is human login only and never supplies Gog tokens.
For website onboarding, invite the person's Google login email and set the Clerk invitation redirect to <website-origin>/oauth/auth/invite. The website sign-in entry is /oauth/auth/start. Keep registration invite-only, email invitations enabled, and Google identity-only sign-in. The invitation email must match the verified Google sign-in email. Existing email-only accounts use the immediate Google setup action; do not send users through Clerk account menus. Invitations do not create owner/editor permissions: a verified person not yet configured sees Waiting for access. Apply owner/editor configuration, then ask them to check access again. Website login is separate from Google resource consent to the application; the broker binds permitted use of the resulting connection to an agent.
