	...
	`,
		...
		},
		{
			...
			content: generatedPage(
				'MCP Portal',
				`
MCP Portal is a scoped tool facade over deployment-owned upstream MCP servers. Managed OpenClaw uses native tools in the gateway VM; external MCP clients use agent-vm-mcp-portal serve.
	...
	Each agent receives a named profile in mcp-portal.config.jsonc. mcp.config.jsonc owns the upstream MCP providers; mcp-portal.config.jsonc owns agent profile assignments and profile policies. Namespace exposure defaults to deny-all unless config enables namespaces. Denied tools do not enter the agent's catalog or search index.
