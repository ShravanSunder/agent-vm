...
Per-agent auth isolation works by using gateway.authProfilesByAgent for OpenClaw auth profiles, agent-vm auth codex-harness for native Codex CLI auth, and first-boot files through agentSandboxSeeds. Seeds target paths relative to the agent sandbox backing directory exposed at /workspace in Tool VMs and do not overwrite existing files.
