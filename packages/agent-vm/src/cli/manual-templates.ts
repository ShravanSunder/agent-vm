...
For website onboarding, invite the person's Google login email and set the Clerk invitation redirect to <website-origin>/oauth/auth/invite. The website sign-in entry is /oauth/auth/start. Keep registration invite-only, email invitations enabled, and Google identity-only sign-in. The invitation email must match the verified Google sign-in email. Existing email-only accounts use the immediate Google setup action; do not send users through Clerk account menus. Invitations do not create owner/editor permissions. Website login is separate from each agent's Google resource consent.
