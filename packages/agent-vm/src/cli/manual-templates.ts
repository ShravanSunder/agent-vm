...
MCP Portal is a scoped tool facade over deployment-owned upstream MCP servers. Managed OpenClaw uses native tools in the gateway VM; external MCP clients use mcp-portal mcp-proxy serve.
	...
	Authored config is JSONC. $schema points at local config/schemas/*.schema.json files generated by agent-vm init. schemaVersion is the runtime migration gate.
...
Use agent-vm controller ssh --zone <zoneId> for a gateway admin shell with runtime environment and OPENCLAW_GATEWAY_TOKEN loaded.
