...
config/gateways/<zone>/oauth.config.jsonc optionally enables controller-owned Google OAuth for that managed Hermes zone. It defines Cloudflare Access issuer/audience identity, human owner/editor subjects, a configurable public HTTPS origin, a loopback HTTP listener, and Google application-family bindings. Agent role policy belongs in the sibling Tool Portal profiles, not in an OAuth agents map. Accounts and separate agent/account/application connections are enrolled dynamically. Google credential payloads are envelope-encrypted in controller-state SQLite using a 1Password-held wrapping key; Access login is human identity only and never supplies Gog tokens.
For website onboarding, configure a Cloudflare Access self-hosted application for the public origin, use Google as the identity provider with Instant Authentication, and allow only intended people. Set the Access application session duration to at least the longest ten-minute ceremony bound; 24 hours is recommended. Enable Protect with Access on the Tunnel route and route only to the configured loopback HTTP listener. The website sign-in entry is /oauth/auth/start. Access admission does not create owner/editor permissions: a verified person not yet configured sees Waiting for access. Configure the person's Access subject, then ask them to check access again. Website login is separate from Google resource consent to the application; the broker binds permitted use of the resulting connection to an agent. Keep the controller administration listener outside the Tunnel route.
tool-portal.config.jsonc owns the finite Gog executable surface and profiles.<profile>.oauthApplications.<applicationId>. Each application entry declares ceiling, optional consentRecommendation, and optional policyDefaults. Use the all-supported catalog preset when the ceiling should not narrow supported permissions. consentRecommendation independently selects a named collection or explicit groupIds; policyDefaults independently selects a named collection or explicit services with Read/Write rules. Omitted recommendations select nothing; omitted default rules deny. Agents select profiles through agents.<agent>.profile. Agents sharing a profile share configured policy, not connected accounts. To migrate, move OAuth agent ceilings and Tool Portal agent googlePolicyDefaults into complete profiles; declare a collection separately for recommendations and defaults. Old property locations are rejected. Validate the config pair before restarting. Explicit website account overrides win independently over those defaults. Deny, Ask, and Allow are valid for both reads and writes; writes are not universally forced to Ask. OAuth consent and exact-call approval remain separate gates.
...
For composition inside Tool VM, use Python connect_tool_portal, TypeScript connectToolPortal, or the tool-portal CLI without transport flags. The managed image installs these interfaces and /agent-vm/tool-portal.md contains examples. Hermes execute_code and foreground terminal supply AGENT_VM_TOOL_PORTAL_SOCKET for the current invocation; child processes inherit it. Composition stays in Tool VM while Tool Portal chooses each call's configured destination. Approval uses the originating conversation's existing human route. The endpoint ends with the invocation: do not reuse it for detached work, supply approval tokens, or replay uncertain effects. Missing context fails explicitly. This relay uses existing SSH process streams, not a public listener, and does not restrict arbitrary direct Tool VM execution.

Managed execute_code starts a fresh remote kernel for every call. In-memory Python variables do not persist between calls; pass results explicitly or use authorized files.

If remote-kernel execution reports an uncertain outcome, verify effects before retrying. The managed Gateway suppresses Hermes's automatic per-call fallback after a possible dispatch.

Managed profiles select catalog presentation with catalogMode: compact or catalog in tool-portal.config.jsonc; omission defaults to compact. Compact keeps the generic Hermes Tool Portal tools, while catalog registers the admitted capabilities as individual Hermes-native tools. Generated TypeScript remains available inside Tool VM in both modes. Managed startup prepares the admitted definitions once, after the private Gateway Runtime connection opens and before Hermes is ready. Catalog-mode preparation failure blocks readiness. The snapshot stays fixed until the managed Gateway restarts: there is no live refresh and no per-Tool-VM rediscovery.

For generated TypeScript, use a foreground Hermes terminal and Node 24. Turn orientation supplies exact factory imports for selected namespaces within its prompt budget. After catalog readiness, AGENT_VM_TOOL_PORTAL_SDK_MANIFEST contains the exact /run/agent-vm/tool-portal-sdk/<definitionFingerprint>/manifest.json path; its namespace entries provide the complete modulePath and exportedFactoryName mapping, including namespaces omitted from orientation. If the variable is absent, generated imports were not admitted for that environment generation. Open one connectToolPortal client, bind each imported factory to it, and inspect the full canonical result including ok, items, and diagnostics. Generated functions still call Tool Portal and cannot bypass provider authentication, policy, approval, or artifact handling. Immutable generated files may be reused when a later invocation's fresh manifest selects the same fingerprint. Never reuse the prior client, socket, or authority. The installed /agent-vm/tool-portal.md guide contains the runnable template and generic-client examples.
	...
	All portal responses are { ok, items, diagnostics? }. Each item has id and status. Successful items use { status: "ok", value }; failed items use { status: "error", error }.
		...
		controller_execution operations are registered_action or configured_cli. Ordinary controller-host and credentialed configured_cli operations use an invocation calls object: calls.deny and calls.requiresApproval match exact admitted command paths plus optional present flags, while calls.withoutApproval must be remaining_admitted. Google account operations instead use calls.source: managed_google_policy, with per-account Read/Write policy resolved by the controller. Tool VM operations use the suggest* fields described below. The fixed result order is deny > requires_approval > without_approval; rule order cannot weaken it. Keep commands[].flagRules only for allowed_values admission and put path-scoped flag denial in configured_cli.calls.deny. A visible configured_cli on the namespace direct baseline still requires zone approvalAccess when its calls.requiresApproval matcher list is non-empty; Hermes is the sole native presenter in this release.

		Configured CLI binds controller_host, a reusable credentialed ephemeral_managed_vm, or the current leased tool_vm. The tool_vm discriminant uses the same configured-CLI policy schemas under suggestCalls, suggestCommands, suggestDeniedPatterns, suggestStdin, and suggestTimeout; suggestCalls contains suggestDeny, suggestRequiresApproval, and suggestWithoutApproval. These names are deliberate: they govern only tool_portal_call and do not restrict terminal, Python, SSH, or other arbitrary execution inside the Tool VM. Successful Tool Portal calls run the configured executable and caller argv through the current Tool VM strict-SSH lease without a controller execution RPC. Approval-required calls still arm their controller-issued reservation through the approval control plane immediately before SSH dispatch. For ephemeral_managed_vm, ephemeral means controller-created, non-durable, and idle-retired; it does not mean one VM per RPC. The controller enforces one current credentialed VM per zone and authenticated agent; targets do not declare runtime ids. Each credentialed target declares exactly one file_binding or http_mediation credentialProjection. File bindings map agent-owned 1Password files into read-only memory paths. HTTP mediation gives the VM opaque placeholders while raw values remain host-side for exact hosts. Ordinary CLI config/state/cache remains on disposable COW. One agent runtime runs one command; concurrent calls return retryable busy and are never queued. Compatible calls reuse the VM until 15 idle minutes, but every call gets current policy and approval. imageReference remains a recipe path prepared at Gateway startup. Prepared image details, credential refs, file paths, and VM authority stay out of Gateway-safe projection. quick operations run for at most 5 seconds. open operations default to 120 seconds and accept a caller timeout up to 8 hours.
	...
	Standalone MCP Portal selects its presentation at process startup with mcp-portal mcp-proxy serve --catalog-mode compact|catalog; omission defaults to compact. This standalone CLI option and mcp-portal.config.jsonc are separate from the managed profile catalogMode and private-UDS authority above.
