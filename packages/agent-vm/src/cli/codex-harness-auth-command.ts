...
function buildCodexHarnessAuthRemoteCommand(agentId: string): string {
	const script = [
		...
		'  codex_js="$({',
		'    if command -v pnpm >/dev/null 2>&1; then pnpm root -g 2>/dev/null || true; fi',
		'    if command -v npm >/dev/null 2>&1; then npm root -g 2>/dev/null || true; fi',
		'    printf "%s\\n" /usr/local/lib/node_modules',
		'  } | while IFS= read -r search_root; do',
		'    [ -n "$search_root" ] || continue',
		'    [ -d "$search_root" ] || continue',
		'    find "$search_root" -path \'*/node_modules/@openai/codex/bin/codex.js\' -type f -print -quit',
		'  done)"',
		'  if [ -z "$codex_js" ]; then',
		'    echo "Could not locate the Codex CLI binary in PATH or pnpm global install paths." >&2',
		'    echo "Install @openai/codex in the OpenClaw gateway image, or add it to the gateway overlay openClawPackageOverrides before running auth codex-harness." >&2',
		'    exit 127',
		'  fi',
