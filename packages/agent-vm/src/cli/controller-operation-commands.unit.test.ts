...
function createTestSystemConfig(
	_gatewayConfigPath: string,
	systemConfigPath: string,
): LoadedSystemConfig {
	return createHermesSystemConfig(
		path.join(
...
describe('runControllerOperationCommand', () => {
	...
	it('writes a human-readable failed doctor summary by default', async () => {
		const outputs: string[] = [];
		const systemConfig = createHermesSystemConfig(
			'/tmp/agent-vm-doctor/tool-vm-build-config.json',
			'/tmp/agent-vm-doctor/system.json',
		);
			...
			dependencies: {
				...
				runControllerDoctor: () => ({
					...
					checks: [
						...
						{ name: 'controller-port', ok: true, value: 18_800 },
						{ name: 'host-cache-dir', ok: true },
						{ name: 'host-runtime-dir', ok: true },
						{ name: 'gateway-image-profile-hermes', ok: true },
					],
				}),
			},
			io: {
		...
		expect(output).toContain('Passing (4)');
	...
	it('uses singular wording for one hidden passing doctor check', async () => {
		...
		try {
			...
			const gatewayConfigPath = path.join(temporaryDirectoryPath, 'worker.json');
			const outputs: string[] = [];
			const systemConfig = createTestSystemConfig(gatewayConfigPath, systemConfigPath);
	...
	it('accepts never-pulled managed Tool VM images without a Dockerfile producer', async () => {
		const temporaryDirectoryPath = await fs.mkdtemp(path.join(os.tmpdir(), 'agent-vm-doctor-'));
		const systemConfigPath = path.join(temporaryDirectoryPath, 'config', 'system.json');
		const toolVmBuildConfigPath = path.join(
			temporaryDirectoryPath,
		...
		const overlayPath = path.join(path.dirname(toolVmBuildConfigPath), 'overlay.jsonc');
		await fs.mkdir(path.dirname(toolVmBuildConfigPath), { recursive: true });
		await fs.writeFile(
			toolVmBuildConfigPath,
			JSON.stringify({ oci: { image: 'agent-vm-tool:latest', pullPolicy: 'never' } }),
		...
		await fs.writeFile(overlayPath, JSON.stringify({ schemaVersion: 1 }), 'utf8');
		const baseConfig = createHermesSystemConfig(toolVmBuildConfigPath, systemConfigPath);
		const systemConfig: LoadedSystemConfig = {
			...
			imageProfiles: {
				...
				toolVms: {
					default: {
						...baseConfig.imageProfiles.toolVms.default,
						buildConfig: toolVmBuildConfigPath,
						source: { base: 'tool-vm', kind: 'managedBase', overlay: overlayPath },
						type: 'toolVm',
					},
				},
			},
		};
		const gatewayBuildConfigPath = systemConfig.imageProfiles.gateways.hermes?.buildConfig;
		...
		await fs.writeFile(
			...
			JSON.stringify({ oci: { image: 'agent-vm-hermes:latest', pullPolicy: 'ifNotPresent' } }),
			'utf8',
		);
		const outputs: string[] = [];
	...
	it('reports missing Hermes image build configs in controller doctor output', async () => {
		const temporaryDirectoryPath = await fs.mkdtemp(path.join(os.tmpdir(), 'agent-vm-doctor-'));
		const systemConfigPath = path.join(temporaryDirectoryPath, 'config', 'system.json');
		const toolVmBuildConfigPath = path.join(
			temporaryDirectoryPath,
		...
		const systemConfig: LoadedSystemConfig = {
			...
			imageProfiles: {
				...
				gateways: {
					hermes: {
						...baseConfig.imageProfiles.gateways.hermes,
						buildConfig: missingGatewayBuildConfigPath,
						type: 'hermes',
					},
				},
			},
		...
		expect(result.ok).toBe(false);
		expect(
			result.checks.find((check) => check.name === 'gateway-image-profile-hermes-build-config'),
		).toMatchObject({ ok: false, hint: expect.stringContaining(missingGatewayBuildConfigPath) });
		await fs.rm(temporaryDirectoryPath, { force: true, recursive: true });
	});
	...
	it('reports missing vm-host-system files for container configs in doctor output', async () => {
		...
		await runControllerOperationCommand({
			...
			json: true,
			subcommand: 'doctor',
			systemConfig: createTestSystemConfig(gatewayConfigPath, systemConfigPath),
		});
