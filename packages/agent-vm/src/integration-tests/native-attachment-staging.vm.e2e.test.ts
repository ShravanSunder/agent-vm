...
describeLiveNativeStaging('native attachment controller staging on real rootfs VMs', () => {
	...
	it.each([false, true])(
		...
		async (failFirst) => {
			...
			const writer = requireManagedFileTransfer(destination);
			...
			try {
				...
				const relativePath = staged.path.slice('/work/'.length);
