			...
			zones: [
				{
					...
					gateway: {
						...
						profileSecretProjectionsByAgent: {
							'first-agent': {
								API_SERVER_KEY: 'API_SERVER_KEY_FIRST',
								DISCORD_BOT_TOKEN: 'DISCORD_BOT_TOKEN_FIRST',
							},
							'second-agent': {
					...
					secrets: {
						...
						API_SERVER_KEY_FIRST: {
							audience: 'gateway',
							envVar: 'API_SERVER_KEY_FIRST',
							injection: 'env',
							source: 'environment',
						},
						...
						DISCORD_BOT_TOKEN_FIRST: {
							audience: 'gateway',
							envVar: 'DISCORD_BOT_TOKEN_FIRST',
							injection: 'env',
							source: 'environment',
						},
...
describeLiveVmE2e('live e2e: controller restart composed cleanup', () => {
	it('fails closed on ambiguity, then destroys C1 Tool leaves before Gateway and publishes C2', async () => {
		...
		if (zone === undefined || zone.gateway.type !== 'hermes') {
			throw new Error('Expected one Hermes controller-restart cleanup zone.');
		}
