...
export function buildPortalCompositionProgram(options: {
	...
	const pythonProgram = [
		...
		'    if re.fullmatch(r"/tmp/hermes_rkernel_[0-9a-f]{12}", os.getcwd()) is None:',
		'        raise RuntimeError("execute_code did not use the stock remote owner-kernel directory")',
		...
		'        tool_vm_calls_path = pathlib.Path("/work/portal-composition-tool-vm-calls.txt")',
		'        tool_vm_calls_path.unlink(missing_ok=True)',
		...
		'        repeated_operation_ids = {tool_vm_item["operationId"]}',
		'        for independent_request in range(2):',
		'            repeated_result = await portal.call({"calls": [{"arguments": {"argv": ["write-tool-vm-effect", derived_value], "reason": "Prove same Tool VM configured CLI routing"}, "id": "python-tool-vm-write", ' +
			`"name": ${JSON.stringify(toolVmEffectOperationName)}, "namespace": ${JSON.stringify(configuredCliNamespace)}}]})`,
		'            repeated_payload = repeated_result.model_dump(by_alias=True, mode="json", exclude_none=True)',
		'            if not repeated_payload["ok"]: raise RuntimeError("independent repeated item did not execute")',
		'            repeated_operation_ids.add(repeated_payload["items"][0]["operationId"])',
		'        if len(repeated_operation_ids) != 3 or tool_vm_calls_path.read_text(encoding="utf-8").splitlines() != [derived_value] * 3: raise RuntimeError("independent requests shared identity or lost an actual Tool VM effect")',
		...
		'        for sequential_read in range(70):',
		`            continued_read = await portal.artifacts.read({"maxBytes": ${String(artifactReadMaximumBytes)}, "offsetBytes": 0, "reference": artifact_reference})`,
		'            continued_proof = verify_artifact_read(continued_read.model_dump(by_alias=True, mode="json", exclude_none=True), artifact_reference, expected_artifact_bytes, expected_artifact_sha256)',
		'            transferred_artifact_bytes += continued_proof["byteLength"]',
		'        if transferred_artifact_bytes <= 64 * 1024 * 1024: raise RuntimeError("capacity reuse proof did not exceed the old cumulative limit")',
