...
describe('volume-manager', () => {
	test('ensureVolumeDir creates a stable directory path', () => {
		const cacheBasePath = fs.mkdtempSync(path.join(os.tmpdir(), 'agent-vm-volume-cache-'));
		createdDirectories.push(cacheBasePath);

		const volumeDirectoryPath = ensureVolumeDir(cacheBasePath, 'workspace-a', 'state');
	...
	test('resolveVolumeDirs maps named volumes to guest paths', () => {
		const cacheBasePath = fs.mkdtempSync(path.join(os.tmpdir(), 'agent-vm-volume-map-'));
		createdDirectories.push(cacheBasePath);

		expect(
			resolveVolumeDirs(cacheBasePath, 'workspace-b', {
