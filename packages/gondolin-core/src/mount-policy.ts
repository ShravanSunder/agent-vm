...
function normalizeHostPath(hostPath: string): string {
	...
	try {
		return fs.realpathSync(resolvedHostPath);
	} catch {
...
export function validateRuntimeMountPolicy(
	...
	for (const [guestPath, hostPath] of Object.entries(config.extraMounts)) {
		validateWritableMount(guestPath, config.mountControls, options);

		if (!config.mountControls.allowAuthWrite && path.isAbsolute(hostPath)) {
			const resolvedWritableHostPath = normalizeHostPath(hostPath);
			const overlapsProtectedHostPath = resolveAuthHostPrefixes(options.hostHome)
