...
Ordinary controller-host and credentialed `configured_cli` operations require
their own invocation-level `calls` policy in addition to namespace call selectors.
Google account operations instead select `calls.source: "managed_google_policy"`,
with per-account Read/Write policy resolved by the controller. Tool VM operations
use the corresponding `suggest*` fields described below. For static policy, namespace
