...
| control upgrade and idle interruption | deferred runtime-owner qualification | the removed OpenClaw transport/lifecycle sequence is not adopted as a new Hermes cutover contract; current Hermes control evidence and the separate reattachment stress remain visible |
...
Inventory-only skips do not satisfy this ledger. The known baseline-red recovery
stress case remains separately runnable and may establish only exact
base-versus-cutover non-regression; it is never relabeled green.
