...
| --- | --- | --- | --- | --- |
| U1 | Tool-backed work must recover automatically after an idle lease expires for managed OpenClaw and Hermes agents. | A call arriving through either framework adapter during or after idle retirement succeeds when a ready binding is available within its existing deadline; otherwise it receives one bounded failure and a later call succeeds automatically, without restarting the Gateway or framework. | One intermediate retirement causes a permanent tools-only outage for either framework. | Critical |
...
- OpenClaw and Hermes consume the same shared retirement correction. Their framework plugins do not become lease or VM lifecycle owners, and each framework requires its own product-path proof.
