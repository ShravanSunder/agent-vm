...
| control-ack-timeout | 2_000 ms | connect(3s)+ack(2s)=5s < active-use-start 10s budget (invariant 2). Runtime must keep the connect/hello budget and per-message command-ack budget as separate knobs so a long enough VM ingress connection window does not silently lengthen control message receipt waits. |
