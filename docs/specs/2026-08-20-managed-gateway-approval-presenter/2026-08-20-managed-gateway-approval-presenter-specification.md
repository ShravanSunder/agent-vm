...
For `ephemeral_managed_vm`, authored `imageReference` is the Managed VM image recipe path, resolved relative to the directory containing `tool-portal.config.jsonc`. Static validation checks that authored path shape. Gateway startup preflight and runtime materialization MUST pass that recipe to the existing `ManagedVmImageCapability.prepareImage` owner and fail when preparation does not produce both a non-empty provider-local image reference and fingerprint. The atomic effective generation replaces the authored recipe path with a strict controller-only prepared-image identity containing exactly that returned reference and fingerprint before admission or approval intent derivation. Loading an effective generation MUST reject an authored recipe string, malformed identity, missing reference, or missing fingerprint. The Gateway projection omits both prepared values. `guestCwd`, the empty/inherited non-secret environment policy, and `allowedHosts` are the target's only other authored runner fields. `allowedHosts` defaults to no network destinations.
