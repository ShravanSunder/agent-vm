...
When `kind: "managed_gateway"` is configured, the selected managed Gateway lifecycle MUST declare that it supplies the native approval presenter capability. Static validation or Gateway preflight MUST reject a worker Gateway, an unsupported managed Gateway, or a lifecycle whose runtime does not provide the declared capability.
...
| V2 | Unsupported Gateways and missing authorities fail before protected execution. | Automated integration and startup/preflight evidence |
| V3 | Native presenter shows one session-and-request-bound approve/deny interaction using the framework API selected by Program Design; an admitted actor can resolve it, while an unauthorized actor receives bounded feedback and leaves it pending. | Real managed-Gateway interaction or visual evidence with admitted and unauthorized native-surface actors |
...
| V18 | `ephemeral_managed_vm` resolves an immutable image, creates one fresh COW/no-mount/no-SSH runner, records identity before dispatch, executes array argv, applies empty/allowlisted environment and allowed-host policy, starts timeout at guest process start, never leases/reuses/adopts, and proves cleanup/containment; dispatch-armed or containment failure remains ambiguous. | Real ManagedVm integration with lifecycle-ledger, provider-request, process, network-denial, and containment observations |
