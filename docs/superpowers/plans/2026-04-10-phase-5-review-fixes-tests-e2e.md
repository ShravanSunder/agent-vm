...
- `gateway-zone-orchestrator.ts` (or its split files): Fix H5 (throw on readiness exhaustion)
...
`waitForGatewayReadiness()` in `gateway-openclaw-lifecycle.ts` returns silently when all attempts are exhausted. The controller proceeds as if OpenClaw started successfully, leading to confusing failures downstream.
...
In `gateway-zone-orchestrator.ts` (or its split files), change the base case of the recursive function:
...
No changes needed to controller-http-routes.ts or controller-lease-client.ts — the existing `GET /lease/:leaseId` and `DELETE /lease/:leaseId` endpoints already accept leaseId directly.
...
In `gateway-vm-setup.ts`, the heredoc writes `$OPENCLAW_GATEWAY_TOKEN` which relies on env inheritance during exec. If the exec context doesn't inherit the env, the token is empty. Additionally, `/etc/profile.d/` is world-readable — secrets must not go there.
...
In `gateway-zone-orchestrator.ts` (or its split files), replace the `/etc/profile.d/openclaw.sh` approach entirely. The env vars file should:
...
- Do NOT extract gateway lifecycle into a separate module — `startGatewayZone` already lives in `gateway-zone-orchestrator.ts` (or its split files). A thin re-export wrapper would be busywork with no distinct responsibility.
...
it('releases a lease at exactly the TTL boundary', async () => {
	const releaseLease = vi.fn(async () => {});
	const reaper = createIdleReaper({
		getLeases: () => [{ id: 'boundary', lastUsedAt: 5_000 }],
		now: () => 10_000,
		releaseLease,
		ttlMs: 5_000,
	});
...
- [ ] **Step 3: Replace manual type guards** with Zod validation in controller-http-routes.ts
...
10. No manual type guards (`isLeaseCreatePayload`, `isDestroyPayload`) remain in controller-http-routes.ts
```
