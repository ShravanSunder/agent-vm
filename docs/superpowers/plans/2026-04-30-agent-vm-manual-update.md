...
This plan does not reconfigure `shravan-claw`. That should be separate
deployment work after this framework command exists.

This plan does not implement filesystem hardening. It only updates manual,
...
Implementation note: the generated manual should mention the current
multi-zone operator model, but schema examples for `agentToolProfiles`,
`agentSandboxSeeds`, `authProfilesByAgent`, and `leaseIdleTtl` must be copied
from landed implementation code, not guessed from design notes.
